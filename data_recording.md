...
This branch (`feature/data-recorder`) introduces a lightweight, non-blocking data recorder to the `stretch_dex_teleop` repository. It is designed to capture synchronized robot telemetry, commanded teleoperation actions, external webcam imagery, and (optionally) wrist and head camera imagery for downstream imitation learning models (such as `stretch_ai` or `LeRobot`).

> **Head camera (D435i) status**: implemented and locally tested (recorder logic + converter, both against synthetic data), but **not yet verified against the physical D435i** — in particular, the 90° rotation correction's direction hasn't been confirmed by eye on a real captured frame yet. Treat head camera images with that caveat until this note is updated.
...
All recorded data will be saved inside the `data/episode_YYYY-MM-DD--HH-MM-SS/` directory, containing `trajectory.csv`, an `images/` folder, (if detected) `wrist_images/` and `head_images/` folders, and (unless `--skip-success` was passed) a `success.txt` label.
...
- **Failed image writes are also dropped, not left dangling**: if a background image write fails (disk full, permissions, or any other unexpected error from `cv2.imwrite`), that frame's row is excluded from `trajectory.csv` at `stop_episode()` rather than being written with a reference to a file that doesn't exist. `stop_episode()` prints a warning listing exactly which frame(s) were dropped and why. This means `trajectory.csv` never lies about which images actually exist on disk — you won't discover a missing-file error for the first time when running the converter, possibly on a different machine days later.
- **Wrist camera (Intel RealSense D405) and head camera (Intel RealSense D435i) are both optional and auto-detected, not configured via a flag**: `dex_teleop.py` tries to initialize each at startup via `pyrealsense2` (no ROS2 dependency), sharing a common `RealSenseCamera` base class (`realsense_camera.py`). If a camera isn't found, teleop proceeds normally without its images (a warning is printed once, at startup). If found, a color frame is grabbed every loop tick — the same synchronous, once-per-tick pattern already used for the external webcam — and saved as JPEG through the same background writer/failure-detection path. Depth is intentionally not captured or stored for either camera. The head camera's frame is rotated 90° before saving, to correct for its physical mounting orientation (unlike the wrist camera, which needs no rotation).
...
- `image_wrist_cam`: Relative path to the saved wrist camera (D405) RGB image (e.g. `wrist_images/wrist_cam_000000.jpg`), or an empty string if no wrist camera was detected during recording. Either every row in an episode has this filled in, or none do — never a mix within one episode.
...
If the episode's `trajectory.csv` has wrist and/or head camera images (`image_wrist_cam`/`image_head_cam` non-empty), the converter automatically adds the corresponding video feature(s) — `observation.images.wrist_cam` (640×480) and/or `observation.images.head_cam` (640×480, rotated) — alongside the existing `observation.image` (1920×1080, the external webcam), following LeRobot's own multi-camera naming convention of `observation.images.<camera_name>`. Episodes recorded before a given camera existed still convert exactly as before, with no corresponding feature in the resulting dataset. You don't need to pass anything for this — it's detected automatically per episode, per camera.
...
- **`robot_type` is not currently set** in the converted dataset's metadata (`info.json` will show `"robot_type": null`) — a deliberate choice for now, not an oversight. It matters for LeRobot's own dataset-aggregation tooling, its native recording script's robot-mismatch check, and as a real model input for multi-embodiment policies, but this pipeline doesn't use any of those paths yet. Worth revisiting once you're training a policy where robot identity should matter (e.g. combining Stretch 2 and Stretch 3 data) — this codebase already distinguishes the two generations elsewhere (`--stretch_2` flag), so a naming convention should account for that rather than assume a single "stretch" type.
- **Image resolution is hardcoded** in `convert_to_lerobot.py` (`(3, 1080, 1920)` for the webcam, `(3, 480, 640)` for the wrist camera, `(3, 640, 480)` for the head camera — note the head camera's dimensions are swapped relative to its raw capture size, since its image is rotated 90° before saving), matching each camera's configured resolution in `webcam_teleop_interface.py` / `wrist_camera.py` / `head_camera.py`. If you change any camera's resolution, update this script to match, or the converter will fail immediately on the first frame with a clear shape-mismatch error (it won't silently corrupt the dataset — LeRobot validates every frame's shape against the declared feature schema).
- **Head camera rotation direction is not yet hardware-verified** — see the note at the top of this document.
