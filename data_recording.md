...
All recorded data will be saved inside the `data/episode_YYYY-MM-DD--HH-MM-SS/` directory, containing `trajectory.csv`, an `images/` folder, (if a wrist camera was detected) a `wrist_images/` folder, and (unless `--skip-success` was passed) a `success.txt` label.
...
- **Wrist camera (Intel RealSense D405) is optional and auto-detected, not configured via a flag**: `dex_teleop.py` tries to initialize the D405 at startup via `pyrealsense2` (no ROS2 dependency). If it's not found, teleop proceeds normally without wrist images (a warning is printed once, at startup). If it is found, a color frame is grabbed every loop tick — the same synchronous, once-per-tick pattern already used for the external webcam — and saved as JPEG through the same background writer/failure-detection path. Depth is intentionally not captured or stored.
...
If the episode's `trajectory.csv` has wrist camera images (`image_wrist_cam` non-empty), the converter automatically adds a second video feature, `observation.images.wrist_cam` (640×480), alongside the existing `observation.image` (1920×1080, the external webcam) — following LeRobot's own multi-camera naming convention of `observation.images.<camera_name>`. Episodes recorded before the wrist camera existed still convert exactly as before, with no wrist feature in the resulting dataset. You don't need to pass anything for this — it's detected automatically per episode.
...
- **Image resolution is hardcoded** in `convert_to_lerobot.py` (`(3, 1080, 1920)` for the webcam, `(3, 480, 640)` for the wrist camera), matching each camera's configured resolution in `webcam_teleop_interface.py` / `wrist_camera.py`. If you change either camera's resolution, update this script to match, or the converter will fail immediately on the first frame with a clear shape-mismatch error (it won't silently corrupt the dataset — LeRobot validates every frame's shape against the declared feature schema).
